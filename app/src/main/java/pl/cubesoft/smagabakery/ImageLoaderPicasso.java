...
public class ImageLoaderPicasso implements ImageLoader {
    ...
    public void load(Uri uri, Transform transform, Object tag, Listener listener) {
        load(uri, transform, tag, null, listener);
    }


    @Override
    public void load(Uri uri, Transform transform, Object tag, Size size, Listener listener) {
        ...
        if (size != null) {
            rq.resize(size.width, size.height);
        }
        rq.into(new Target() {
            @Override
